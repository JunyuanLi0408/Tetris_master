...
def board_reverse(board):  # 直接对棋盘进行操作
    for i in board:
        i.reverse()
    board.reverse()
    return board
...
class Decision_tree:
    ...
    def evaluate(self, matchdata):  # 返回当事人视角下的局面估值
    ...
    def minmaxsearch(self, matchdata, max_depth, top_n):
        # 返回当前局面的估值，在搜索时每层的子局面只考虑最好的top_n个局面
        if self.depth == max_depth:
            return self.evaluate(matchdata)  # 搜索深度达到上限直接返回估值

        else:
            newboard = self.update_board(matchdata)  # 更新得到各自视角下的棋盘
            nextboard = board_reverse(newboard)  # 子树会轮到另一方落块，切换board视角
            all_pos = matchdata.getAllValidAction(self.nextblock, nextboard)  # 另一方落块所有可能的操作
            value_lst = {}  # 字典储存每个子节点的估值
            value_lst_shortsighted = {}
            if len(all_pos) > 0:
                for i in range(len(all_pos)):
                    child_i_temp = Decision_tree(all_pos[i], nextboard, self.blocklst, self.depth + 1, self)  # 创建子树
                    shortsighted_i = child_i_temp.evaluate(matchdata)
                    value_lst_shortsighted[child_i_temp] = shortsighted_i

                value_lst_shortsighted2 = sorted(value_lst_shortsighted.items(), key=lambda x: x[1], reverse=True)
                for i in range(int(min(len(all_pos), top_n))):  # 只考虑最好的top_n个局面
                    child_i = Decision_tree(value_lst_shortsighted2[i][0].blockpos \
                                            , nextboard, self.blocklst, self.depth + 1, self)  # 创建子树
                    value_i = child_i.minmaxsearch(matchdata, max_depth, top_n)
...
class Player:
    ...
    def output(self, matchdata):
        ...
        for i in range(len(validpos)):
            case_i = Decision_tree(validpos[i], nowboard, blocklst, 1)
            shortsighted_values[case_i] = case_i.evaluate(matchdata)
        value_lst_shortsighted2 = sorted(shortsighted_values.items(), key=lambda x: x[1], reverse=True)

        for i in range(int(min(len(validpos), top_n))):
            child_i = Decision_tree(value_lst_shortsighted2[i][0].blockpos, nowboard, blocklst, 1, this_case)
            values[child_i] = child_i.minmaxsearch(matchdata, max_depth, top_n)
            print(child_i.blockpos, values[child_i])
        value_lst = sorted(values.items(), key=lambda x: x[1], reverse=True)
        print('finally choose', value_lst[0][0].blockpos)
